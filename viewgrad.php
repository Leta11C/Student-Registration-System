...
<?php $get_id = $_GET['id']; ?>
<?php
			$student_query=mysql_query("select * from studentt where uid = '$get_id' and year = 'Third year' and semester='1st'")or die(mysql_error());
   ...
   <table cellpadding="0  cellspacing="0" border="0" class="table  table-bordered" id="example" >                      
	...
	<?php if ($grade == 'A+'){ ?>
	 ...
	 <?php $get_id = $_GET['id']; ?>
<?php
			$student_query=mysql_query("select * from studentt where uid = '$get_id' and year='Third year' and semester = '2nd'")or die(mysql_error());
			while($row = mysql_fetch_array($student_query)){
			$school_year = $row['year'];
			$term = $row['semester'];
			$student_status = $row['status'];
